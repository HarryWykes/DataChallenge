...
ui <- fluidPage(
  ...
  # Application title
  titlePanel("Levels of Cholesterol and RestingBP on Heart Disease (0/1)"),
  ...
  fluidRow(
    column(width=4,
           ...
           radioButtons("variablechoice", "Choice of variable:", 
                        choices = c("RestingBP", "Cholesterol"))
...
server <- function(input, output) {
  ...
  output$distPlot <- renderPlot({
    vble <- input$variablechoice
    ggplot(cleaned_heartdata, aes(x=.data[[vble]], 
                                  colour=HeartDisease, 
                                  fill=HeartDisease)) +
